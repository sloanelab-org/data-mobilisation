...
public class MainController {
	...
	public String processImage(@RequestParam("image") String imagePath,@RequestParam("vol") String volume) throws IOException {
		    return ProcessPages.process(imagePath,volume,region,access,secret);
	}
	
	@GetMapping("/processImagesFolder")
	public String processImagesFolder(@RequestParam("folder") String inputFolder,@RequestParam("vol") String volume) throws IOException {
	    ...
	    for (File file:files) {
	    	if (file.getName().endsWith(".jpg")) {
	    	  try {	
		        String outcome = ProcessPages.process(file.getPath(),volume,region,access,secret);
